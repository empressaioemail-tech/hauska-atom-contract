...
describe("parseInlineReferences", () => {
  ...
  it("round-trips a colon-bearing entityId (the case the `|` delimiter exists to support)", () => {
    // Real-shape entityIds embed `:` themselves (parcel-briefing, intent,
    // briefing-source, neighboring-context). The `:`-delimited token
    // shape collided with these — the third+ colon in the input was
    // treated as a delimiter, splitting the id across the entityId and
    // displayLabel slots. The `|` delimiter makes them round-trip
    // losslessly. This test is the positive proof: if a future change
    // reintroduces a `:`-sensitive parser, this assertion fails before
  ...
  it("old-shape {{atom:...}} tokens do not parse (no dual-parse contract)", () => {
    // Contract enforcement. The token shape changed from
    // `{{atom:type:id:label}}` (colon-delimited) to
    // `{{atom|type|id|label}}` (pipe-delimited) precisely so colon-
    // bearing entityIds stop colliding with the delimiter.
