...
import type { AtomRegistration, AtomReference } from "./registration.js";
...
describe("resolveComposition", () => {
  ...
  it("silently skips forwardRef edges whose child is still unregistered", () => {
    // A parent that opts an edge into `forwardRef` is opting in to
    // "produce zero children for this edge until the child catalog atom
    // registers" rather than crashing the resolver. The other (concrete)
    // edge must still resolve normally.
    const registry = createAtomRegistry();
