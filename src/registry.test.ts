...
describe("createAtomRegistry", () => {
  ...
  it("validate skips composition edges marked forwardRef", () => {
    // A parent may declare a child atom slated for a later sprint via
    // `forwardRef: true`. The boot validator must treat that edge as
    // deliberate and not as a dangling reference.
    const registry = createAtomRegistry();
    registry.register(
