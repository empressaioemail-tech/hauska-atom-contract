 ...
 * editor lets developers register atoms that violate the contract.
 *
...
describe("type-level registration contract", () => {
  ...
  it("a registration missing composition fails to typecheck", () => {
    // The composition field is required (every registration must declare
    // the composition layer of the four-layer contract — pass
    // `composition: []` to declare 'no children').
  ...
  it("a ContextSummary missing historyProvenance fails to typecheck", () => {
    // The history anchoring layer of the four-layer contract is surfaced
    // into context through `ContextSummary.historyProvenance` — a
    // required field carrying { latestEventId, latestEventAt } (and
    // optional chainHash) sourced from the EventAnchoringService.
